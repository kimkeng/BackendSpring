...
<body>
	...
	<table border="1">
		...
		<tbody>
			...
			try{
				...
				//out.print("페이지 번호 오류");
				//try성공 -> finally작동됨, try실패 -> catch -> finally
			}finally{	//try후에는 finally가 무조건 작동됨
				if(now_page == 0){
					now_page = 1;
				}
			}
			out.print(now_page);
			%>
			<app:set var="now" value="<%=now_page%>"/>
			<app:set var="w" value="${total_person-(now*2)+2}"></app:set>
	...
	<!-- 페이지 번호 forEach의 기본은 배열이 기본으로 작동되는 원리-->
